// Vendored verbatim from reactbits.dev's own "Manual" install tab
// (https://reactbits.dev/animations/pixel-swap, upstream source:
// github.com/DavidHDev/react-bits, src/content/Animations/PixelSwap) --
// same manual-copy fallback every other reactbits component in this project
// uses, because the `npx shadcn@latest add @react-bits/...` CLI has a known
// environment bug on this machine/Node version (see build-log.md's earlier
// rounds for StarBorder/SpecularButton/BorderGlow/etc.). Unlike those
// components, PixelSwap has zero extra npm dependencies (no gsap/ogl) -- it
// only uses React state/refs and the native Web Animations API
// (`element.animate`), so nothing new was added to package.json for this.
// No source edits made here; all site-specific wiring (the scroll-driven
// `active` trigger, the sr-only real heading/tagline, sizing) lives in
// Hero.jsx and index.css instead, matching how BorderGlow/SpecularButton
// stay unmodified and get configured entirely via props + surrounding CSS.
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }) => {
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    window.push({
      offset: progress,
      // THIRD-ROUND OVERLAP FIX (Facilitator, live-verified): the stock 1.6
      // multiplier keeps a pixel partially transparent for most of its own
      // local animation (opacity only reaches 1 once this pixel's eased
      // progress crosses 1/1.6 = 62.5%). Since pixels are staggered across
      // the whole grid's `spread` window, at almost any moment during the
      // transition a large fraction of pixels are still ramping -- letting
      // the fully-opaque outgoing layer show through everywhere, which is
      // the "text overlaps" bug. A solid pixel background alone (see
      // index.css) doesn't fix this: it only hides what's underneath a
      // pixel that has ALREADY reached full opacity, not one still fading
      // in. Raising the multiplier makes each pixel reach full opacity
      // almost as soon as its own local animation starts (eased>=1/8=12.5%)
      // instead of over half its lifetime, while keeping `fade`'s opacity-0
      // starting state (via `fill:'both'`) -- so there's still no instant
      // "all pixels pop at once" artifact (that was `fade:false`'s failure
      // mode, tried and reverted in the prior round), just a much shorter
      // per-pixel fade window.
      opacity: fade ? Math.min(1, eased * 8) : 1,
